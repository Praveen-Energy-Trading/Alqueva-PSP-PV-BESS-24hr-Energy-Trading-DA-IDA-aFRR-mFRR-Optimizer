...
(fetch_ida3_prices / forecast_ida3_prices) — OMIE never publishes IDA3's own
future clearing price. What CAN be live is the model's TRAINING data:
update_training_data() below backfills real settled IDA3 clearing prices for
past dates, falling back to synthetic spreads on any download failure.

Live source (same file family as IDA1/IDA2, session 03 — see
omie_ida1_price_loader.py's module docstring for the full format spec):
    https://www.omie.es/es/file-download?parents=marginalpibcpt&filename=marginalpibcpt_<YYYYMMDD>03.1
...
_TRAINING_XLSX = os.path.join(_HERE, "ida3_training_data_2024_2025.xlsx")
_TRAINING_SHEET = "IDA3_2024_2025"
...
# ---------------------------------------------------------------------------
# Training-data live update — backfills real settled IDA3 clearing prices
# ---------------------------------------------------------------------------

def update_training_data(delivery_date: str) -> None:
    """Fill ida3_training_data Excel with real (or synthetic) IDA3 prices up
    ...
    for dt in missing:
        ...
        try:
            ...
            source = "OMIE_LIVE"
            log.info(f"  {date_str} IDA3 -> OMIE_LIVE downloaded")
        except Exception as exc:
            ida_prices = _synthetic_ida_prices(date_str, _HOURS, da_prices)
            source = "SYNTHETIC"
            log.warning(f"  {date_str} IDA3 -> OMIE failed ({exc}), using SYNTHETIC")
    ...
    log.info(f"IDA3 Excel updated: {len(missing)} date(s) added, "
...
def _download_omie_ida(delivery_date: str, session: str,
    ...
    hour. IMPORTANT: IDA3's real file only ever contains periods 49-96 (48
    rows, hours 13-24) — hours 1-12 are never published, since IDA3 doesn't
    clear trades for them (verified against multiple live dates). We do NOT
    require all 96 periods; we build the hourly dict from whichever hours
    have a complete set of 4 quarters, and the caller (update_training_data)
    falls back to the DA price for any hour this leaves missing. Raises only
    if the file is empty/near-empty (network issue, wrong filename, garbage
    response) — a genuinely partial-but-real file is not a failure.
    """
    import requests
...
def _synthetic_ida_prices(delivery_date: str, hours: List[int],
    ...
    import random
    rng = random.Random(f"ida3-{delivery_date}")
    return {
